...
bool Market::insert(const Order &o) {
  if(o.getSide()==Order::buy) {
    double p = (o.getType()==Order::market ? BID_INFINITY : o.getPrice());
    m_bidOrders.insert({ p, o });
  } else {
    double p = (o.getType()==Order::market ? ASK_INFINITY : o.getPrice());
    m_askOrders.insert({ p, o });
  }
  return true;
}
...
void Market::match(Order &bid, Order &ask) {
  double price;
  if(bid.getType()==Order::market && ask.getType()==Order::limit)
  price = ask.getPrice();
  else if(bid.getType()==Order::limit && ask.getType()==Order::market)
  price = bid.getPrice();
  else                         
  price = ask.getPrice();
